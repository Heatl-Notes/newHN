...
async function loadPatients() {
  ...
  patientsJson.forEach((patient) => {
    ...
    if (
      patientComorbidities.length > 0 &&
      patientComorbidities[0].description !== ""
    ) {
    ...
    if (
      ...
      patientProcedures[0].description !== ""
    ) {
...
function showPatientProfile(patient) {
  ...
  const removePatientButton = popupElemento.querySelector(
    ".removePatientButton"
  );
...
async function removePatient(patient) {
  ...
  const removePatient = await fetch(
    `${apiUrl}/caregiver/${currentClient.cpf}/patient/${patient.cpf}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
